...
REM - kopiert WPF-Setup zusaetzlich ins Repo KGV-WPF
...
if defined ISCC if defined WPF_ISS (
  ...
  for /f "usebackq delims=" %%I in (`powershell -NoProfile -ExecutionPolicy Bypass -Command ^
    "$repo = '%REPO%';" ^
    "$setup = Get-ChildItem $repo -Recurse -Include *.exe | Where-Object { $_.FullName -notmatch '\\bin\\|\\obj\\' -and $_.Name -match 'setup' } | Sort-Object LastWriteTime -Descending | Select-Object -First 1 -ExpandProperty FullName;" ^
    "if ($setup) { Write-Output $setup }"`) do set "SETUP_FILE=%%I"
) else (
  echo.
...
if defined SETUP_FILE (
  copy /Y "%SETUP_FILE%" "%WPF_SETUP_OUT%\KGV.Setup-%TARGET_VERSION%.exe" >nul || (
    echo FEHLER: Setup konnte nicht nach %WPF_SETUP_OUT% kopiert werden.
    exit /b 1
  )
  copy /Y "%SETUP_FILE%" "%VERSION_ROOT%\KGV.Setup-%TARGET_VERSION%.exe" >nul || (
    echo FEHLER: Setup konnte nicht nach %VERSION_ROOT% kopiert werden.
    exit /b 1
  )
  copy /Y "%SETUP_FILE%" "%WPF_RELEASE_REPO%\KGV.Setup-%TARGET_VERSION%.exe" >nul || (
    echo FEHLER: Setup konnte nicht als versionsierte Datei ins KGV-WPF Repo kopiert werden.
    exit /b 1
  )
  copy /Y "%SETUP_FILE%" "%WPF_RELEASE_REPO%\KGV-Setup.exe" >nul || (
    echo FEHLER: Setup konnte nicht als KGV-Setup.exe ins KGV-WPF Repo kopiert werden.
    exit /b 1
  )
) else (
  echo FEHLER: Keine erzeugte WPF-Setup.exe gefunden. Ohne Setup wird nicht weitergemacht.
  exit /b 1
)
