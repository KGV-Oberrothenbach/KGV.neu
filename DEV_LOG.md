...
- Minimal umgesetzt:
  - `KGV.Wpf/Views/ImpressumView.xaml`
    - `BooleanToVisibilityConverter` lokal als View-Ressource mit Key `BoolToVisibilityConverter` ergänzt
    - Demo-Schalter an `IsDemoToggleVisible` und `ShowDemoData` aus dem bestehenden ViewModel gebunden
    - damit entfällt der fehleranfällige externe Ressourcenpfad im View-Kontext
  - `KGV.Maui/Pages/ImpressumPage.cs`
    - Admin-Sichtbarkeit des vorhandenen Demo-Schalters über `IsDemoToggleVisible` zentralisiert
    - Vorstand sieht den Schalter nicht; ohne Admin-Rolle wird der Switch auf `false` zurückgesetzt
    - sichtbare Kontakte laufen weiter über `OperationalDataFilter.IsOperationalImpressumKontakt(...)`
- Fachliche Wirkung:
  - WPF-Impressum öffnet ohne `XamlParseException` durch fehlende `StaticResource`
  - WPF und MAUI nutzen denselben bestehenden Demo-/Operativfilterpfad
  - ohne explizites Einschalten bleiben Demo-Datensätze ausgeblendet
- Validierung:
  - `dotnet build KGV.Wpf/KGV.Wpf.csproj` erfolgreich
  - `dotnet build KGV.Maui/KGV.Maui.csproj` erfolgreich
  - keine passenden automatisierten `Impressum`-Tests im Test Explorer gefunden
...
- Kurzer Repo-Check vor dem Block:
  ...
  - blockfremd untracked blieben Bilddateien, `PS_Log.bat`, `_logs/`, `_secrets/`
- Geprüfter Iststand:
  - WPF-Impressum referenzierte in `KGV.Wpf/Views/ImpressumView.xaml` einen `BoolToVisibilityConverter` per `StaticResource`, ohne dass dieser dort verfügbar war
  - Folge war ein Laufzeitfehler beim Öffnen der View (`XamlParseException` / Resource nicht gefunden)
  - in WPF waren `IsDemoToggleVisible` und `ShowDemoData` im `ImpressumViewModel` bereits vorhanden
  - in MAUI fehlte der Demo-Schalter auf `KGV.Maui/Pages/ImpressumPage.cs` noch komplett
- Minimal umgesetzt:
  - `KGV.Wpf/Views/ImpressumView.xaml`
    - lokalen `BooleanToVisibilityConverter` als Ressource ergänzt und bestehendes Binding weiterverwendet
  - `KGV.Maui/Pages/ImpressumPage.cs`
    - Admin-only-Schalter `Demo-Datensätze einblenden` ergänzt
    - Standardzustand bleibt aus
    - sichtbare Kontakte laufen über denselben bestehenden `OperationalDataFilter.IsOperationalImpressumKontakt(...)`-Pfad wie in WPF
- Fachliche Wirkung:
  - WPF-Impressum öffnet wieder ohne Ressourcenfehler
  - WPF und MAUI verhalten sich auf der Impressum-Seite jetzt gleich: nur Admin sieht den Schalter, Vorstand nicht; ohne Einschalten bleiben Demo-Daten ausgeblendet
- Reale Validierung dieses Laufs:
  - `dotnet build KGV.Wpf/KGV.Wpf.csproj`
  - `dotnet build KGV.Maui/KGV.Maui.csproj`
...
- Direkt geprüft wurden in diesem Block nur die vorgegebenen Android-Packaging-Dateien:
  ...
  - `DEV_LOG.md`
  - `KGV_Fortschrittslog_ausfuehrlich.md`
- Ehrlicher Packaging-Befund:
  - `AndroidLinkTool=r8` und `AndroidEnableProguard=true` wurden erst mit `a40f4a1` vorbereitet und mit `4508ca9` aktiv in den Release-Pfad von `KGV.Maui.csproj` aufgenommen
  - Debug verwendet diesen Shrinker-Pfad nicht; dort bleibt `AndroidLinkMode=None` ohne R8/ProGuard aktiv
  - das passt exakt zum Fehlerbild: Debug-Build grün, Release-Packaging entfernt den Java-Typ `com.microsoft.maui.PlatformDispatcher`, wodurch `VisualElement` schon in `UseMauiApp<App>()` scheitert
  - `AWR.bat` wurde nur geprüft; der Batch behandelt fehlende Mapping-/Symbolartefakte bereits tolerant und musste für diesen Fix nicht geändert werden
- Minimal umgesetzt:
  - `KGV.Maui/KGV.Maui.csproj`
    - `AndroidLinkTool=r8` aus dem Release-Pfad entfernt
    - `AndroidEnableProguard=true` aus dem Release-Pfad entfernt
    - übrige Release-/Diagnoseeinstellungen wie `AndroidIncludeDebugSymbols=true` unverändert belassen
  - damit ist der Android-Shrinker-/ProGuard-Pfad auf den letzten stabilen Zustand zurückgeführt, ohne neue Fachlogik oder Shell-/Root-Umbauten zu eröffnen
- Reale Packaging-Validierung nach dem Fix:
  - `dotnet build KGV.Core/KGV.Core.csproj -c Debug`
  - `dotnet build KGV.Wpf/KGV.Wpf.csproj -c Debug -clp:ErrorsOnly`
  - `dotnet build KGV.Maui/KGV.Maui.csproj -c Debug -clp:ErrorsOnly`
  - `dotnet build KGV.Maui/KGV.Maui.csproj -c Release -f net9.0-android -clp:ErrorsOnly`
  - zusätzlicher Paketcheck am erzeugten Android-Artefakt:
    - `com.microsoft.maui.PlatformDispatcher` ist nach dem Fix wieder im finalen APK vorhanden (`classes2.dex`)
- Ehrliche Wirkung dieses Laufs:
  - die echte innere Ursache war ein Android-Packaging-/Shrinker-Problem, nicht Shell/Root/Fachlogik
  - der Release-Pfad entfernt `PlatformDispatcher` jetzt nicht mehr; damit ist genau der bekannte Startup-Verursacher code-seitig geschlossen
  - der finale Geräte-Start selbst ist in diesem Headless-Lauf weiterhin nicht direkt ausgeführt, aber das Paket enthält die vorher fehlende Klasse jetzt wieder belastbar
...
- Ausgangspunkt dieses Laufs war weiterhin derselbe echte Android-Startup-Crash direkt im sehr frühen MAUI-Controls-Pfad:
  ...
  - danach harter Crash:
    - `System.TypeInitializationException`
    - `The type initializer for 'Microsoft.Maui.Controls.VisualElement' threw an exception.`
    - Stack weiter bei `RemapForControls` / `UseMauiApp` / `MauiProgram.CreateMauiApp`
- Direkt geprüft wurden diesmal nur die vorgegebenen Blockdateien und der unmittelbar angebundene Resource-/Packaging-Pfad:
  - `KGV.Maui/KGV.Maui.csproj`
  - `KGV.Maui/MauiProgram.cs`
  - `KGV.Maui/App.xaml`
  - `KGV.Maui/App.xaml.cs`
...
- Ausgangspunkt dieses Laufs war erneut derselbe echte Android-Normalstart-Crash im frühen Startup-Pfad:
  - `System.TypeInitializationException`
  - `The type initializer for 'Microsoft.Maui.Controls.VisualElement' threw an exception.`
  - `UseMauiApp` / `MauiProgram.CreateMauiApp` / `MainApplication.CreateMauiApp`
- Direkt geprüft wurden nur die vom Auftrag vorgegebenen Blockdateien:
  - `KGV.Maui/MauiProgram.cs`
  - `KGV.Maui/App.xaml.cs`
  - `KGV.Maui/AdminShell.cs`
  - `KGV.Maui/UserShell.cs`
  ...
  - `DEV_LOG.md`
  - `KGV_Fortschrittslog_ausfuehrlich.md`
- Ehrlicher Istbefund dieses Laufs auf dem aktuellen Workspace-`main`:
  - der frühere synchrone Root-Aufbau in `App.CreateWindow(...)` war im aktuellen Workspace bereits nicht mehr der alte Problemstand; `App.xaml.cs` startet schon mit einer Bootstrap-Seite und verschiebt den echten Root-Wechsel per `Task.Yield()`
  - `MainActivity.cs` war ebenfalls bereits auf dem robusteren Android-Pfad mit `base.OnCreate(null)` und ohne zusätzlichen frühen Shell-Startup-Code
  - als verbleibender früher Restpfad blieb damit vor allem der erste Shell-`Loaded`-Tick:
    - `AdminShell` initialisierte Menü/Kontext/Route weiterhin sofort im ersten Dispatcher-Aufruf
    - `UserShell` initialisierte denselben frühen Pfad ebenfalls noch ohne zusätzlichen Yield-Tick
    - genau dort konnte weiterhin zu früh Flyout-/Shell-`VisualElement`-Struktur im direkten Android-Startupfenster entstehen
- Minimal zusätzlich umgesetzt:
  - `KGV.Maui/AdminShell.cs`
    - vor `ClearImplicitOwnMemberContext()`, `BuildMenu()` und den nachgelagerten Shell-Refreshs jetzt explizit `await Task.Yield()` im `Loaded`-Pfad
  - `KGV.Maui/UserShell.cs`
    - denselben ersten `Loaded`-Pfad ebenfalls auf `Dispatcher` + `await Task.Yield()` gezogen, bevor eigener Kontext, Menü und Route aufgebaut werden
  - `KGV.Maui/App.xaml.cs`
    - nur geprüft; der Bootstrap-/deferred-root-Fix war hier bereits vorhanden und wurde in diesem Lauf nicht nochmals umgebaut
  - `KGV.Maui/MainActivity.cs`
    - nur geprüft; kein zusätzlicher Startup-/Lifecycle-Code gefunden, der den frühen Root-/Shell-Aufbau in diesem Block weiter verschärft
- Bewusst nicht gemacht:
  - keine neue Fachlogik
  - kein neuer Shell-/Routing-Großumbau
  - keine neue Schattennavigation
  - kein Eingriff in blockfremde Module
- Echte Validierung dieses Laufs:
  - `dotnet build KGV.Core/KGV.Core.csproj -c Debug`
  - `dotnet build KGV.Wpf/KGV.Wpf.csproj -c Debug -clp:ErrorsOnly`
  - `dotnet build KGV.Maui/KGV.Maui.csproj -c Debug -clp:ErrorsOnly`
...
## 2026-04-07 – MAUI-Startup-Crash im Android-Normalstart weiter auf frühen Root-/Shell-VisualElement-Pfad eingegrenzt und zusätzlich entschärft
...
- Direkt geprüft wurden nur die blockrelevanten Start-/Shell-/Resource-Dateien:
  ...
  - `DEV_LOG.md`
  - `KGV_Fortschrittslog_ausfuehrlich.md`
- Echter Istbefund dieses Laufs:
  - `AppShell.xaml` existiert im aktuellen Repo nicht; ein separater globaler Shell-XAML-Pfad ist damit aktuell nicht die Ursache
  - `App.xaml` ist weiterhin praktisch leer und wird im aktuellen Pfad nicht als belastbarer globaler Resource-/Style-Initialisierer verwendet
  - `Colors.xaml` ist zwar weiterhin ein auffälliger Altbestand mit WPF-Namespaces, wird im aktuellen aktiven Startup-Pfad aber nirgends global gemerged und war damit für den echten Crash dieses Laufs nicht die belastbar aktive Ursache
  - der vorhandene Debug-Log lief real erneut bis mindestens:
    - `APP_START`
    - `APPSETTINGS_LOAD_OK`
...
- Minimal zusätzlich umgesetzt:
  - `KGV.Maui/App.xaml.cs`
    ...
    - der echte Root-Wechsel auf Login/Shell wird jetzt erst nach Window-Erstellung per `MainThread` + `Task.Yield()` ausgeführt
...
- Echte Validierung dieses Laufs:
  ...
  - `dotnet build KGV.Wpf/KGV.Wpf.csproj -c Debug -clp:ErrorsOnly` erfolgreich
  - `dotnet build KGV.Maui/KGV.Maui.csproj -c Debug -clp:ErrorsOnly` erfolgreich
  - MAUI blieb dabei nur mit bereits vorhandenen Warnungen grün, u. a. in `MainActivity.cs`, `HomeManagementPage.cs` und `ImpressumPage.cs`
- Ehrliche Grenze dieses Laufs:
...
- Kurz geprüft wurden nur die direkt relevanten Dateien:
  - `AWR.bat`
  - `DEV_LOG.md`
  - `KGV_Fortschrittslog_ausfuehrlich.md`
- Ehrlicher Iststand dieses Abschlusslaufs:
  - `AWR.bat` ist bereits erweitert
  - pro Release gibt es jetzt den festen Diagnoseordner `publish\<Version>\Android\GooglePlay-Diagnose`
  - vorhandene Mapping-Dateien werden dort unter `Mapping` gesammelt
  - vorhandene native Debug-Symbole werden dort unter `NativeDebugSymbols` gesammelt
  - `STATUS.txt` dokumentiert pro Release sauber, wenn diese Artefakte im aktuellen Build nicht entstehen
  - aktuell erzeugt der Android-/MAUI-Build weiterhin keine app-spezifische Mapping-Datei und keine nativen Debug-Symbol-Artefakte
- In diesem Lauf bewusst nicht gemacht:
  - keine weitere Android-Build-Großbaustelle eröffnet
  - keine Änderung an App-Fachlogik
  - keine Änderung an Android-Runtime-/Packaging-Properties außerhalb des bereits vorhandenen Batch-Blocks
- Ergebnis dieses Abschlusslaufs:
  - der Block ist jetzt in den Logs sauber auf den echten Stand nachgezogen
  - der Release-Prozess ist für spätere Play-Console-Uploads vorbereitend ergänzt, auch wenn der aktuelle Build selbst Mapping/Symbole noch nicht liefert
...
- Minimal umgesetzt:
  - `KGV.Core/Models/MembershipEndDecision.cs`
  - `KGV.Core/Models/MembershipEndResult.cs`
...
- Minimal umgesetzt:
  - `ToDo.md`
    - vollständig auf den aktuellen Prompt umgestellt
...
- Fachlicher Zwischenstand dieses Blocks:
  - Nebenmitglieder können auf Serviceebene jetzt als eigenständige Wartungsvertrags-Träger behandelt werden
  - WPF und MAUI profitieren ohne getrennte Schattenpfade vom gemeinsamen Shared-Service
  - Restprüfung des Gesamtverhaltens für Pflichtstunden- und UI-Kontexte bleibt offen und ist deshalb im Zielbild bewusst nur als `teilweise` markiert
- Validierung im Lauf wirklich ausgeführt:
  - `dotnet build KGV.Core/KGV.Core.csproj -c Debug`
  - `dotnet build KGV.Wpf/KGV.Wpf.csproj -c Debug -clp:ErrorsOnly`
  - `dotnet build KGV.Maui/KGV.Maui.csproj -c Debug -clp:ErrorsOnly`
...
- Ehrlicher Restbefund vor dem Fix:
  - die Einstiegspfade in MAUI waren bereits verbessert
  ...
  - schreibende Aktionen waren bereits korrekt an `CanManageDocuments` gebunden
- Minimal umgesetzt:
  - `KGV.Maui/Pages/DokumentePage.xaml.cs`
    ...
    - Seite blendet Liste/Refresh/Upload jetzt passend zum echten Lesekontext
...
- Fachliches Ergebnis dieses Laufs:
  ...
  - nicht freigegebene Dokumentkontexte werden klar gesperrt
- Validierung im Lauf wirklich ausgeführt:
  - `dotnet build KGV.Maui/KGV.Maui.csproj -c Debug -clp:ErrorsOnly`
  - `dotnet build KGV.Wpf/KGV.Wpf.csproj -c Debug -clp:ErrorsOnly`
...
- Übernommen wurde minimal-invasiv:
  - `KGV.Wpf/ViewModels/AblesenOverviewViewModel.cs`
  ...
  - `KGV.Maui/Pages/AblesenOverviewPage.cs`
    - Admin-Schalter für Nutzer-Ablesungen wirksam berücksichtigt
    - sichtbare Tiles/Hinweise an den effektiven Freigabestand gebunden
  - `KGV.Maui/Pages/HomePage.xaml.cs`
...
- Direkt geprüft wurden:
  ...
  - `KGV.Maui/ShellRouteRegistrar.cs`
  - `KGV.Maui/ShellNavigationHelper.cs`
  - gegen den Folgekontext zusätzlich geprüft:
    - `KGV.Maui/Pages/MeineDatenPage.xaml.cs`
    - `KGV.Maui/Pages/MemberGardensPage.cs`
    - `KGV.Maui/Pages/DokumentePage.xaml.cs`
    - `KGV.Maui/Pages/UserManagementPage.cs`
...
- Ehrlicher Befund vor dem Fix:
  - die Mitgliedsauswahl setzte den `SelectedMember` bereits korrekt in `MemberContextState`
  - der anschließende Einstieg in `Stammdaten` lief aber noch über `Shell.Current.GoToAsync("//memberdetails")`
  - `memberdetails` ist in `AdminShell` ein dynamisch sichtbarer mitgliedsbezogener Shell-Eintrag, der erst nach gesetztem Mitgliedskontext sichtbar wird
  - der Navigationspfad verließ sich damit auf Shell-Routenauflösung in genau dem Moment, in dem die Sichtbarkeit des Zielpunkts gerade erst umgeschaltet wurde
  ...
  - Folgepfade wie `Stammdaten`, `Gärten des Mitglieds`, `Dokumente`, `Arbeitsstunden` und `Benutzerverwaltung` selbst arbeiten bereits korrekt auf `MemberContextState.SelectedMember`; dort war kein zusätzlicher Umbau nötig
- Minimal umgesetzt:
  - in `KGV.Maui/Pages/MemberSearchPage.xaml.cs` nach gesetztem Mitgliedskontext nicht mehr primär per absoluter Shell-Route navigiert
  - stattdessen wird jetzt der bereits vorhandene Shell-Eintrag `memberdetails` direkt über `ShellNavigationHelper.EnsureActiveShellItem(shell, "memberdetails")` aktiviert
  - nur wenn dieser direkte Stammdaten-Einstieg wider Erwarten nicht aktiviert werden kann, bleibt der vorhandene Fallback auf `MeineDatenPage` bestehen
  - keine Änderung an `MemberContextState`, Shell-Struktur, Garten-/Dokumente-/Arbeitsstundenpfaden oder WPF
- Ergebnis:
  - `Mitgliedersuche -> Mitglied auswählen` öffnet im Admin-/Mitgliedskontext jetzt direkt die Stammdatenansicht des ausgewählten Mitglieds
  - kein unnötiger Zwischensprung auf `Home`
  - der gesetzte Mitgliedskontext bleibt für die vorhandenen Unterpunkte erhalten
- Validierung:
  - `dotnet build KGV.Maui/KGV.Maui.csproj -c Debug -clp:ErrorsOnly`
  - `dotnet build KGV.Wpf/KGV.Wpf.csproj -c Debug -clp:ErrorsOnly`
  - kein echter Laufzeittest in dieser Umgebung; nur code-/build-seitige Validierung
...
- Git-Befund zu Beginn:
  - `main` liegt auf `origin/main`
  - Divergenz `origin/main...HEAD` => `0 0`
  - bereits offen im Arbeitsbaum lagen weiter die aktuellen Prompt-1/3-Dateien:
    - `DEV_LOG.md`
    - `KGV.Infrastructure/Services/SupabaseService.cs`
    - `KGV.Maui/Pages/ArbeitseinsaetzeEditorPage.cs`
    - `KGV.Maui/Pages/BekanntmachungEditorPage.cs`
...
- Direkt geprüft wurden:
  ...
  - `KGV.Core/Interfaces/ISupabaseService.cs`
- Ehrlicher Befund vor dem Fix:
  - auf der WPF-Startseite lag der Scroll-Fehler nicht in fehlenden Daten, sondern im Layout:
    - `Arbeitseinsätze` hatte zwar bereits einen `ScrollViewer`, dieser hing aber innerhalb eines `StackPanel` und erhielt dadurch keine sinnvolle Höhenbegrenzung
    - `Termine` und `Bekanntmachungen` nutzten in der Home-Ansicht noch gar keinen eigenen `ScrollViewer`
  - beim Arbeitseinsatz wurde im aktuellen Produktivpfad service-seitig bereits das echte Feld `datum` auf `Subtitle` gemappt
  - die sichtbare Restkante war damit kein falsches Servicefeld `anmeldung_bis`, sondern die zu unklare unlabeled Anzeige der Datumszeile auf Home
  - direkte Löschpfade existierten im Shared-Service bereits:
    - `DeleteBekanntmachungAsync(long id)`
    - `DeleteTerminAsync(long id)`
    - `DeleteArbeitseinsatzAsync(long id)`
  - in den WPF-Verwaltungseditoren fehlte dafür aber noch die echte UI-/Command-Verdrahtung inklusive Sicherheitsabfrage und Reload
- Minimal umgesetzt:
  - `HomeView.xaml` so korrigiert, dass die drei Startseitenbereiche `Arbeitseinsätze`, `Termine` und `Bekanntmachungen` jetzt in einem höhenbegrenzten Grid-Bereich mit eigenem `ScrollViewer` laufen statt bei vielen Einträgen abgeschnitten zu wirken
  - im Arbeitseinsatz-Block der Startseite wird das Datum jetzt explizit als `Einsatzdatum` gerendert; damit ist fachlich sichtbar klar, dass das echte Einsatzdatum gezeigt wird und nicht eine Fristinformation
  - `BekanntmachungenVerwaltungViewModel`, `TermineVerwaltungViewModel` und `ArbeitseinsaetzeVerwaltungViewModel` um echten `Löschen`-Command ergänzt
  - Löschpfad nutzt dabei ausschließlich die bereits vorhandenen Shared-Service-Delete-Methoden; keine neue CRUD-Architektur gebaut
  - vor dem Löschen wird jeweils eine Sicherheitsabfrage angezeigt
  - während des Löschens wird der Löschpfad lokal gegen Mehrfachauslösung gesperrt
  - nach erfolgreichem Löschen wird der Editor geschlossen, die betroffene Verwaltungsliste neu geladen und eine kurze Erfolgsmeldung angezeigt
  - die drei WPF-Editorviews erhielten dafür jeweils einen direkten Button `Löschen` im bestehenden Aktionsbereich; sichtbar nur für bestehende Datensätze, nicht im `Neu`-Modus
- Ergebnis:
  - die WPF-Startseite bleibt bei vielen Einträgen in `Arbeitseinsätze`, `Termine` und `Bekanntmachungen` sauber scrollbar
  - Arbeitseinsätze zeigen auf Home sichtbar das echte Einsatzdatum
...
- Direkt geprüft wurden:
  ...
  - `KGV.Maui/Pages/HomeSectionDetailPage.cs`
- Ehrlicher Befund vor dem Fix:
  - ein echter Save-Fehler lag nicht in jedem gemeldeten Fall vor
  - Root Cause 1: die Shared-Create-Pfade für `Bekanntmachung`, `Termin` und `Arbeitseinsatz` behandelten das unmittelbare Re-Read nach erfolgreichem Insert faktisch als Pflichtteil des Saves
  - wenn dieses strenge Re-Read keinen exakten Treffer lieferte, kam `null` zurück, obwohl der Datensatz real bereits gespeichert war; WPF und MAUI zeigten dadurch eine falsche Fehlermeldung
  - Root Cause 2: im MAUI-Arbeitseinsatz-Editor liefen Follow-up-Schritte wie Reload der Verwaltungsnavigation und Rücknavigation noch im selben Fehlerpfad wie das eigentliche Speichern; ein Fehler danach wurde dadurch als Save-Fehler gezeigt, obwohl das Update schon persistiert war
...
- Minimal umgesetzt:
  - `SupabaseService` behandelt erfolgreiche Inserts für `Bekanntmachung`, `Termin` und `Arbeitseinsatz` jetzt tolerant: wenn das optionale direkte Re-Read keinen exakten Match liefert, wird nicht mehr fälschlich `null` und damit kein falscher Save-Fehler zurückgegeben
  - MAUI-Editoren für `Bekanntmachung`, `Termin` und `Arbeitseinsatz` trennen jetzt echten Save-Erfolg von optionalem Folge-Reload/Navigation
  - Folgefehler nach erfolgreichem Save führen damit nicht mehr zu einer falschen Meldung `konnte nicht gespeichert werden`
  - `HomePage` nutzt jetzt einen zentralen Reloadpfad für Initialladung und manuelles Aktualisieren
  - auf der MAUI-Startseite wurde Pull-to-Refresh über `RefreshView` ergänzt und an denselben vorhandenen Home-Reloadpfad gebunden
  - der bestehende Invalidate-/Home-Reload-Mechanismus bleibt erhalten; es wurde keine neue Home-Architektur gebaut
- Ergebnis:
  - Save erfolgreich => keine falsche Fehlermeldung mehr wegen strengem Nachladepfad
  - nach Save bleibt die Rücknavigation auf `Home`/Startseite erhalten
  - MAUI-Startseite kann zusätzlich manuell per Nach-unten-Ziehen aktualisiert werden
- Validierung:
  - `dotnet build KGV.Wpf/KGV.Wpf.csproj -c Debug -clp:ErrorsOnly` => erfolgreich
  - `dotnet build KGV.Maui/KGV.Maui.csproj -c Debug -clp:ErrorsOnly` => erfolgreich
...
- Direkt geprüft wurden:
  ...
  - exemplarisch kamera-/pickerrelevante MAUI-Seiten
- Ehrliche Befundliste vor dem Fix:
  - Bereits ok:
    - `ApplicationId` vorhanden: `de.kgv.oberrothenbach`
    - Versionen vorhanden: `ApplicationDisplayVersion=0.3.5`, `ApplicationVersion=48`
    - Release-AAB-Format im Projekt vorhanden
    - notwendige Basis-Permissions für Netzwerk/Kamera/NFC vorhanden
    - Dokument-/Foto-/Picker-Pfade nutzen bestehende MAUI-/MediaPicker-Pfade und wirken im Code nicht als neuer Play-Blocker
  - Echter Blocker:
    - `dotnet publish KGV.Maui/KGV.Maui.csproj -f net9.0-android -c Release -p:AndroidPackageFormat=aab` schlug real fehl mit `AndroidSignPackage ... KeyStore-Parameter`
    - Ursache: `KGV.Maui.csproj` erzwingt im Release-Kontext `AndroidKeyStore=True`, obwohl nicht immer vollständige Signing-Daten gesetzt sind
    - zusätzliche Release-Restlücke: die vorhandenen Batch-Skripte nutzten einen veralteten harten Repo-Pfad und übergaben nur `AndroidSigningStorePass`, aber nicht `KeyStore`, `KeyAlias`, `KeyPass`
  - Nice-to-have, nicht in diesem Block:
    - bestehende MAUI-Warnungen in `HomeManagementPage.cs` und `ImpressumPage.cs`
    - optische Store-/Launcher-Icon-Endprüfung auf Gerät/Play Console
    - tiefergehende ANR-/Resume-Analyse
- Minimal behoben:
  - `KGV.Maui.csproj` so angepasst, dass Release-Signing nur noch dann aktiviert wird, wenn vollständige Signing-Daten vorhanden sind
  - optionaler repo-relativer Keystore-Pfad `_secrets\Android\kgv-upload.keystore` und Default-Alias `kgvupload` hinterlegt, ohne Zwang für normale Release-Validierungsbuilds
  - `Android_release_batch.bat` auf aktuellen Repo-Root umgestellt und vollständige Signing-Parameter ergänzt
  - `Android_Wpf_release_batch_v3.bat` analog auf aktuellen Repo-Root und vollständige Signing-Parameter ergänzt
- Validierung:
  - `dotnet build KGV.Maui/KGV.Maui.csproj -c Debug -clp:ErrorsOnly` => erfolgreich
  - `dotnet publish KGV.Maui/KGV.Maui.csproj -f net9.0-android -c Release -p:AndroidPackageFormat=aab -clp:ErrorsOnly` => erfolgreich
  - `dotnet build KGV.Wpf/KGV.Wpf.csproj -c Debug -clp:ErrorsOnly` => erfolgreich
- Verbleibende echte Play-/Testrisiken:
  - fuer den finalen Store-Upload wird weiterhin ein echter Keystore + Passwort benoetigt; die technische Vorbereitung ist jetzt vorhanden, das Secret selbst bleibt bewusst außerhalb des Repos
  - bestehende nicht-blockierende Warnungen im MAUI-Code sind noch vorhanden
...
- Direkt geprüft:
  ...
  - `KGV.Maui/ShellRouteRegistrar.cs`
  - `KGV.Maui/MauiProgram.cs`
- Gesamtzielbild der Folgeblöcke festgehalten:
  - eine Shell pro Sitzung
  - Rootwechsel nur für Login/Logout/Resume-Timeout
  - Mitgliedswechsel innerhalb derselben Shell
- Ehrlicher Befund:
  - `MemberSearchPage` und `MeineDatenPage` nutzten für Mitgliedswechsel noch `SwitchToCurrentRootAsync(...)`
  - damit war normaler Mitgliedswechsel noch fälschlich an einen echten Rootwechsel gekoppelt
- Umgesetzt:
  - Rootwechsel aus `MemberSearchPage` entfernt; direkte In-Shell-Navigation in die Stammdaten
  - Rootwechsel aus verknüpftem Mitgliedswechsel in `MeineDatenPage` entfernt; Stammdaten laden dort direkt neu
  - `AdminShell` hält die mitgliedsbezogenen Menüpunkte jetzt ohne Shell-Neuaufbau über Sichtbarkeit am `MemberContextState` nach
- Wichtig:
  - `SwitchToCurrentRootAsync(...)` bleibt für Session-/Rootwechsel erhalten
  - kein großer Shell-Umbau vorgezogen
  - keine WPF-Datei geändert
- Validierung:
  - `dotnet build KGV.Maui/KGV.Maui.csproj -c Debug -clp:ErrorsOnly` => erfolgreich
