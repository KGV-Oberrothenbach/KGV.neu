-- Garten-Nummern fachlich sortieren (2 vor 10; 41A direkt bei 41).
...
language sql stable security invoker as $$
  ...
  from rows
  order by case when nullif(garten_nr, '') is null then 1 else 0 end, substring(garten_nr from '^[[:space:]]*([0-9]+)')::integer nulls last,
    garten_nr, nachname, vorname, sortierung, leistendes_mitglied, datum;
$$;
