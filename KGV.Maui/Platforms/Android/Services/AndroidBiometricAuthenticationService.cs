...
public sealed class AndroidBiometricAuthenticationService : IBiometricAuthenticationService
{
    ...
    private static Task<bool> AuthenticateWithPromptAsync(global::Android.App.Activity activity, string reason)
    {
        ...
        var prompt = new BiometricPrompt.Builder(activity).SetTitle("KGV-App entsperren").SetSubtitle(reason)
            .SetNegativeButton("Abbrechen", executor, new CancelListener(source)).Build();
        prompt.Authenticate(new CancellationSignal(), executor, new ResultListener(source));
        return source.Task;
    }
