...
- Direkt geprüft wurden nur die ausdrücklich freigegebenen Dateien und der unmittelbar nötige Shared-Abgleich:
  - `KGV.Maui/Pages/NebenmitgliedPage.cs`
...
- Ehrlicher Iststand vor dem Fix:
  - der DTO-/Create-/Servicepfad war bereits vorbereitet und produktiv vorhanden:
    - `NebenmitgliedCreateDTO` enthält schon `Email`, `Geburtsdatum`, `MitgliedSeit` und `WhatsappEinwilligung`
    - `SupabaseService.CreateNebenmitgliedAsync(...)` übernimmt diese Felder bereits in den Insert-Payload
  - in `NebenmitgliedPage` waren die Controls für:
    - `E-Mail-Adresse`
    - `Geburtsdatum`
    - `Beginn`
    - `WhatsApp`
    bereits instanziiert
  - sichtbar eingebunden waren sie aber noch nicht; die Create-Maske zeigte bisher nur Vorname/Nachname und Adressübernahme, während Telefon/Handy/Adresse außerhalb des Create-Abschnitts lagen
- Minimal umgesetzt:
  - `KGV.Maui/Pages/NebenmitgliedPage.cs`
...
- Direkt geprüft wurden nur die ausdrücklich freigegebenen Dateien:
...
- Ehrlicher Istzustand vor dem Fix:
  - das interne Diagnose-Log unter `FileSystem.AppDataDirectory` war auf nicht debuggablem Android-Paket ohne `run-as` nicht lesbar
  - damit war der vorhandene frühe MAUI-Startup-/Crash-Logpfad auf echten Geräten zwar da, aber praktisch schlecht auslesbar
- Minimal umgesetzt:
  - `KGV.Maui/Services/Diagnostics/AppFileLog.cs`
    - interner Logpfad bleibt unverändert führend
    - zusätzlich wird jetzt in einen extern lesbaren Android-App-spezifischen Pfad gespiegelt, sofern `GetExternalFilesDir(null)` verfügbar ist
    - Spiegelziel:
      - `/storage/emulated/0/Android/data/de.kgv.oberrothenbach/files/diagnostics/kgv-release.log`
    - Spiegel-Fehler werden einmalig protokolliert, ohne das interne File-/`Debug`-/`stderr`-Logging zu beschädigen
    - vollständige Exception-Dumps werden ebenfalls in das externe Spiegel-Log geschrieben
  - `KGV.Maui/MauiProgram.cs`
    - früher Startup-Hinweis erweitert, damit interner und externer Logpfad direkt im Log sichtbar sind
- Bewusst nicht gemacht:
...
- Echte Validierung dieses Laufs:
  - `dotnet build KGV.Wpf/KGV.Wpf.csproj -c Debug -clp:ErrorsOnly`
  - `dotnet build KGV.Maui/KGV.Maui.csproj -c Debug -clp:ErrorsOnly`
- Praktischer Gerätepfad nach diesem Lauf:
...
## 2026-04-07 – MAUI-Android-Startup weiter auf den echten Minimalpfad eingegrenzt und frühes `liblog`-/InnerException-Logging gehärtet
...
- Ausgangspunkt dieses Laufs war weiterhin derselbe echte Android-Startup-Crash direkt im frühen MAUI-Controls-Pfad:
  ...
  - Stack weiter bei:
    - `Microsoft.Maui.Controls.Hosting.AppHostBuilderExtensions.RemapForControls`
    - `SetupDefaults`
    - `UseMauiApp`
    - `KGV.Maui.MauiProgram.CreateMauiApp`
    - `KGV.Maui.MainApplication.CreateMauiApp`
  - zusätzlich direkt davor:
    - `monodroid-assembly: Shared library 'liblog' not loaded, p/invoke '__android_log_print' may fail`
- Direkt geprüft wurden in diesem Block nur die vorgegebenen Startdateien plus die direkt daran angebundenen Logging-/Startup-Pfade:
  - `KGV.Maui/MauiProgram.cs`
  - `KGV.Maui/MainApplication.cs`
  - `KGV.Maui/KGV.Maui.csproj`
  - `KGV.Maui/App.xaml`
...
- Ehrlicher Isolationsbefund dieses Laufs:
  ...
  - `RunStartupStep("USE_MAUI_APP_MINIMAL", ...)` liegt in `MauiProgram` bewusst vor allen späteren Registrierungen; damit sind folgende Pfade als Primärverursacher ausgeschlossen, weil sie beim Crash noch gar nicht erreicht werden:
    - `ShellRouteRegistrar.RegisterCommonRoutes()`
    - `builder.Logging.AddDebug()` / `AppFileLoggerProvider`
    - `AddAppSettings(...)`
    - `ValidateSupabaseConfiguration(...)`
    - `AppSettings.Load()`
    - `RegisterStateServices(...)`
    - `builder.Services.AddKgvServices(...)`
    - `RegisterPageServices(...)`
  - `App.xaml` bleibt weiterhin praktisch leer; im aktuell aktiven Startpfad gibt es dort keinen global gemergten Resource-/Style-Block, der als neue frühe Ursache greifbar wäre
  - `App.xaml.cs` enthält ebenfalls keinen zusätzlichen frühen Controls-/Resource-Block vor `UseMauiApp<App>()`
  - der zuletzt entfernte frühe `VisualElement`-Probezugriff war damit nicht die Hauptursache; der Crash bleibt ohne diesen Probezugriff identisch im frühen MAUI-Controls-Startup bestehen
- Reale zusätzliche Diagnoseursache gefunden:
  - der frühe Logging-Pfad war selbst problematisch für die Fehlerdiagnose
  - `AppFileLog` versuchte bisher zuerst nach `Android.Util.Log` zu schreiben und schrieb Datei-/Fallback-Ausgaben erst danach
  - wenn der frühe Android-`liblog`-Pfad in diesem Startup-Fenster noch nicht belastbar verfügbar ist, konnte dadurch genau die eigentlich wichtige `InnerException`-/`ToString()`-Diagnose verloren gehen
  - der `liblog`-Hinweis ist damit aktuell belastbar als Diagnoseproblem eingeordnet, nicht als nachgewiesene eigentliche `VisualElement`-Hauptursache
- Minimal umgesetzt:
  - `KGV.Maui/Services/Diagnostics/AppFileLog.cs`
    - Logging schreibt jetzt zuerst belastbar in Datei, `Debug` und `stderr`
    ...
    - `ErrorDetailed(...)` schreibt jetzt zusätzlich den vollständigen Exception-`ToString()`-Dump in Datei und `stderr`, damit `InnerException` nicht mehr im frühen Android-Startup verloren geht
  - `KGV.Maui/MauiProgram.cs`
    - `UseMauiApp` jetzt explizit als minimaler Isolationsschritt `USE_MAUI_APP_MINIMAL` markiert
    - wenn genau dieser Schritt scheitert, wird zusätzlich ausdrücklich protokolliert, dass spätere Registrierungen noch gar nicht erreicht wurden
...
- Konkreter Verursacherstand nach diesem Lauf:
  - konkret ausgeschlossen sind jetzt die späteren Registrierungen in `MauiProgram` (Routen, Logging-Provider, Konfiguration, DI, Seiten)
  - konkret gefunden ist ein echter Diagnoseblocker im frühen `liblog`-/Android-Logging-Pfad
  - noch nicht konkret gefunden ist die eigentliche innere `VisualElement`-Ursache innerhalb des minimalen `UseMauiApp<App>()`-/MAUI-Controls-Pfads; genau diese soll der nächste echte Android-Log mit nun belastbarer `InnerException` sichtbar machen
- Bewusst nicht gemacht:
...
- Ehrliche technische Grenze dieses Laufs:
  ...
  - konkret verbessert wurde aber die echte Ursachen-Sichtbarkeit: frühes `liblog` kann die `InnerException` jetzt nicht mehr still verschlucken; der nächste echte Android-Start sollte deshalb den verbleibenden Controls-Verursacher deutlich belastbarer zeigen
...
- Direkt geprüft wurden nur die vorgegebenen Blockdateien:
  ...
  - `DEV_LOG.md`
  - `KGV_Fortschrittslog_ausfuehrlich.md`
- Git-/Repo-Befund dieses Laufs:
  - aktueller Branch: `main`
  - `HEAD` und `origin/main` waren identisch
  - als letzter stabiler Stand vor den 07.04.-Startup-/Shell-Umbauten wurde `7a9a4ac` verwendet
  - gegen diesen Stand ergab sich im aktiven frühen Startup-Pfad:
    - `KGV.Maui/MauiProgram.cs` unverändert
    - `KGV.Maui/MainActivity.cs` unverändert
    - die echte Änderungslinie lag nur in:
      - `KGV.Maui/App.xaml.cs`
      - `KGV.Maui/AdminShell.cs`
      - `KGV.Maui/UserShell.cs`
- Belastbare Einordnung der verdächtigen Linie:
  - `AdminShell`/`UserShell` wurden am 07.04. zwar in den `Loaded`-Pfad verschoben und zusätzlich verzögert, diese Logik greift aber erst nach Shell-Erzeugung
  - der früheste aktive Unterschied im Android-Normalstart war der neue Bootstrap-/deferred-root-Pfad in `App.xaml.cs`
  - zusätzlich lag im aktuellen Workspace genau dort noch ein weiterer uncommitteter Ausbau des Delays vor (`Bootstrap-Seite` + echter Root-Wechsel erst nach `Appearing`)
  - genau dieser Pfad wirkt am frühesten im normalen Android-Startup und ist damit der plausibelste kleine Auslöser für einen Crash, der weiter sehr früh bei `UseMauiApp` / `CreateMauiApp` sichtbar wird
- Minimal umgesetzt:
  - `KGV.Maui/App.xaml.cs`
    - den kompletten Bootstrap-/deferred-root-Pfad auf den stabilen Vor-07.04-Stand `7a9a4ac` zurückgeführt
    - `CreateWindow(...)` erstellt wieder direkt `new Window(CreateRootPage())`
    - `CreateRootPage(...)` baut Login/Shell wieder synchron und setzt die anfängliche Zielroute sofort über `ShellNavigationHelper.EnsureActiveShellItem(...)`
    - die 07.04-Mechanik `SetPreferredStartupRoute(...)`/Bootstrap-Seite/`Task.Yield()` greift damit im frühesten Startup nicht mehr
- Bewusst nicht gemacht:
  - `MauiProgram.cs` nicht verändert, weil im Vergleich zum stabilen Stand keine neue Startup-Differenz vorlag
  - `MainActivity.cs` nicht verändert, weil dort gegenüber `7a9a4ac` ebenfalls keine neue Differenz vorlag
  ...
  - keine Schattennavigation
- Echte Validierung dieses Laufs:
  - `dotnet build KGV.Core/KGV.Core.csproj -c Debug` erfolgreich
  - `dotnet build KGV.Wpf/KGV.Wpf.csproj -c Debug -clp:ErrorsOnly` erfolgreich
  - `dotnet build KGV.Maui/KGV.Maui.csproj -c Debug -clp:ErrorsOnly` erfolgreich
- Ehrliche technische Grenze dieses Laufs:
  - der echte Android-Normalstart-Crash blieb headless nicht direkt reproduzierbar
  - code-seitig wurde deshalb gezielt der im Git-/Diff-Vergleich früheste und plausibelste 07.04-Auslöser zurückgeführt: der Bootstrap-/deferred-root-Startup-Pfad in `App.xaml.cs`
...
- Direkt geprüft wurden nur die Blockdateien und die realen Android-Ausgabepfade:
  - `AWR.bat`
  - `DEV_LOG.md`
  - `KGV_Fortschrittslog_ausfuehrlich.md`
...
- Minimal umgesetzt wurde nur der Batchpfad:
  - neuer Zielordner `publish\<Version>\Android\Upload`
  - Sammlung der im aktuellen Build praktisch nutzbaren Upload-Dateien mit versionsbezogenen Namen:
    ...
    - `README.txt`
  - die vorhandene Diagnose-Sammellogik unter `Android\GooglePlay-Diagnose` blieb erhalten und wurde nicht umgebaut
  - falls kein fertiges Native-Symbol-ZIP vorliegt, wird die Upload-ZIP jetzt aus dem bereits eingesammelten Diagnoseinhalt erzeugt
- Echte Validierung dieses Laufs:
  ...
  - der reale Publish-Output enthielt weiterhin die praktisch relevanten Dateien `de.kgv.oberrothenbach-Signed.apk`, `de.kgv.oberrothenbach-Signed.aab` und `mapping.txt`
  - der reale Native-Symbolpfad `KGV.Maui\obj\Release\net9.0-android\app_shared_libraries` war erneut vorhanden
  - der volle `AWR.bat`-Lauf wurde bewusst nicht automatisiert erzwungen, weil er interaktiv Zielversion und Signaturpasswort verlangt und zusätzlich WPF-/Git-Nebenpfade startet
  - stattdessen wurde der neue Android-Teilpfad des Batchs mit echten Buildartefakten gegen einen temporären Release-Zielordner validiert
  - Ergebnis dieses echten Upload-Checks:
    - `KGV-Android-0.4.5.apk`
    - `KGV-Android-0.4.5.aab`
    - `KGV-Android-0.4.5-mapping.txt`
    - `KGV-Android-0.4.5-native-debug-symbols.zip`
    - `README.txt`
  - der Upload-Ordner enthielt dabei genau `5` Dateien; blockfremde Zusatzdateien wurden nicht mitkopiert
- Fachlogik blieb vollständig unverändert.

## 2026-04-06 – Android-Release für Google-Play-Diagnoseartefakte minimal aktiviert

- Ausgangspunkt dieses Laufs war der bereits vorbereitete AWR-/Diagnoseartefakte-Block: der Batchpfad war fertig, aber der Android-/MAUI-Release erzeugte die benötigten Artefakte noch nicht.
- Direkt geprüft wurden nur die blockrelevanten Dateien und die unmittelbare Android-Release-Konfiguration:
  - `KGV.Maui/KGV.Maui.csproj`
  - `AWR.bat`
  - `DEV_LOG.md`
  - `KGV_Fortschrittslog_ausfuehrlich.md`
- Echter Konfigurationsstand vor der Änderung:
  - `AndroidLinkMode=None`
  - `AndroidLinkTool` leer
  - `AndroidIncludeDebugSymbols=false`
  - `MonoSymbolArchive=false`
  - im realen Release entstand deshalb keine app-spezifische Mapping-Datei und kein durch `AWR.bat` sammelbarer Native-Symbolpfad
- Minimal umgesetzt wurde nur der technisch nötige Android-Release-Block:
  - in `KGV.Maui/KGV.Maui.csproj`
    - `AndroidLinkTool=r8`
    - `AndroidEnableProguard=true`
    - `AndroidIncludeDebugSymbols=true`
  - `AndroidLinkMode=None` blieb absichtlich stehen, damit kein zusätzlicher Managed-Linking-/Trim-Block eröffnet wird
  - in `AWR.bat`
    - die bestehende Native-Diagnosesuche wurde nur um den realen Buildordner `app_shared_libraries` ergänzt
- Echte Build-/Publish-Befunde nach der Änderung:
  - `dotnet build KGV.Maui/KGV.Maui.csproj -c Release -clp:ErrorsOnly` erfolgreich
  - `dotnet publish KGV.Maui/KGV.Maui.csproj -c Release -f net9.0-android -p:AndroidPackageFormat=aab -clp:ErrorsOnly -v:q` erfolgreich
  - `dotnet build KGV.Wpf/KGV.Wpf.csproj -c Release -clp:ErrorsOnly` im Abschlusslauf erneut erfolgreich
  - der WPF-Release-Build lief mit bereits vorhandenen Warnungen in `BekanntmachungenVerwaltungViewModel`, `ArbeitseinsaetzeVerwaltungViewModel` und `TermineVerwaltungViewModel`, aber ohne blockbezogene Folgefehler
  - die Mapping-Datei wird jetzt real erzeugt unter:
    - `KGV.Maui\bin\Release\net9.0-android\mapping.txt`
    - `KGV.Maui\bin\Release\net9.0-android\publish\mapping.txt`
  - native Debug-Symbole entstehen mit dem aktuellen Setup nicht als separates `mSYM`-/`symbols.zip`-Artefakt, sondern als symbolhaltige `.so`-Bibliotheken unter:
    - `KGV.Maui\obj\Release\net9.0-android\app_shared_libraries\arm64-v8a\...`
    - `KGV.Maui\obj\Release\net9.0-android\app_shared_libraries\x86_64\...`
  - kurzer Abschluss-Check gegen den aktuellen Output bestätigt zusätzlich:
    - `KGV.Maui\bin\Release\net9.0-android\mapping.txt` vorhanden
    - `KGV.Maui\obj\Release\net9.0-android\app_shared_libraries` vorhanden, aktuell mit `6` Dateien
  - `AWR.bat` übernimmt diesen Pfad jetzt sauber in den Release-Diagnoseordner `publish\<Version>\Android\GooglePlay-Diagnose\NativeDebugSymbols`
- Ehrlich festgehaltene technische Grenze dieses Laufs:
  - `MonoSymbolArchive=true` wurde testweise geprüft, aber in der aktuellen Umgebung nicht aktiviert, weil der Build damit am fehlenden SDK-Tool `mono-symbolicate.exe` scheitert
  - deshalb wurde bewusst kein zusätzlicher Tooling-/SDK-Reparaturblock eröffnet
- Fachlogik blieb vollständig unverändert.
...
- Ausgangspunkt dieses Laufs war ein eng zusammenhängender MAUI-/Nebenmitglied-Restblock mit vier realen Ursachen:
  - im Admin-Shell-Kontext konnten mitgliedsspezifische Unterpunkte sichtbar werden, obwohl noch kein Mitglied aus der Suche wirklich ausgewählt war
  - das Android-Hardware-Back griff nicht robust genug global in die vorhandene Shell-Backlogik ein
  - im mobilen Nebenmitglied-Create-Modus fehlten noch `Geburtsdatum`, `Beginn`, `E-Mail-Adresse` und `WhatsApp`
  - der Shared-Create-Pfad setzte `mitglied_seit` noch nicht und lief dadurch in den echten DB-Fehler `23502`
- Direkt geprüft wurden in diesem Lauf nur die Blockdateien:
  - `KGV.Maui/AdminShell.cs`
  - `KGV.Maui/MainActivity.cs`
  - `KGV.Maui/UserShell.cs`
  - `KGV.Maui/ShellNavigationHelper.cs`
  ...
  - `KGV.Infrastructure/Services/SupabaseService.cs`
  - `KGV.Maui/Pages/NebenmitgliedPage.cs`
  - ergänzend nur die direkt nötigen Kontext-/Startdateien zur Absicherung des Back- und Shellpfads
- Minimal umgesetzt:
  - in `KGV.Maui/AdminShell.cs`
    - kleiner Guard gegen einen impliziten eigenen Stub-Mitgliedskontext ergänzt
    - mitgliedsspezifische Flyout-Unterpunkte bleiben weiterhin streng an `SelectedMember` gebunden
    - keine automatische Eigenkontext-Setzung im Admin-Shell-Pfad
  - in `KGV.Maui/MainActivity.cs`
    - Hardware-Back delegiert jetzt belastbarer an die aktive Window-/Shell-Seite statt nur an das erste Window
    - dadurch greift die bereits vorhandene Shell-Backlogik globaler und stabiler
  - in `KGV.Core/Models/NebenmitgliedCreateDTO.cs`
    - Kontakt-, Adress-, E-Mail-, Geburtsdatum-, Beginn- und WhatsApp-Felder ergänzt
  - in `KGV.Infrastructure/Services/SupabaseService.cs`
    - `CreateNebenmitgliedAsync(...)` validiert jetzt auch den Pflichtwert `MitgliedSeit`
    - Insertpayload setzt jetzt `mitglied_seit`, `geburtsdatum`, `email`, `whatsapp_einwilligung` sowie Kontakt-/Adressfelder konsistent
    - Adressübernahme bleibt derselbe bestehende Produktivpfad für Adresse/PLZ/Ort
  - in `KGV.Maui/Pages/NebenmitgliedPage.cs`
    - Create-UI um `Geburtsdatum`, `Beginn`, `E-Mail-Adresse` und `WhatsApp` ergänzt
    - Adressübernahme bleibt Vorbelegung statt Zwang
    - Create-Aufruf nutzt jetzt den vollständigen DTO-Pfad; ein nachgelagerter Kontakt-Schattenpfad entfällt
- Fachliche Wirkung dieses Blocks:
  - im Admin-Shell-Kontext erscheinen mitgliedsspezifische Unterpunkte nicht mehr wegen eines impliziten Eigenkontexts
  - Android-Back geht von Unterseiten zuerst sauber nach `home`; nur auf `home` selbst greift die bestehende Exit-Abfrage
  - Nebenmitglied-Anlage in MAUI ist fachlich vollständiger und läuft auf demselben Shared-Create-Pfad wie der Rest des Blocks
  - der bisherige Insertfehler `null value in column "mitglied_seit"` ist code-seitig geschlossen
...
- Minimal umgesetzt:
  - in `KGV.Maui/State/MemberSearchRefreshState.cs`
    - kleine Reload-Flag für die Mitgliedersuche ergänzt
  - in `KGV.Maui/MauiProgram.cs`
    - Reload-State als Singleton registriert
  - in `KGV.Maui/ViewModels/MemberSearchViewModel.cs`
...
- Zu Beginn wurde der echte Repo-Stand gegen `ToDo.md` geprüft:
  ...
  - die echte Restlücke lag damit nicht mehr in fehlender Funktion, sondern in der noch nicht klar genug gemachten Produktivnutzung dieses Minimalpfads
- Minimal umgesetzt:
  - in `KGV.Wpf/ViewModels/ArbeitseinsaetzeVerwaltungViewModel.cs`
    - Editor-Hinweistext um die offizielle Mehrschicht-Nutzung über separate Arbeitseinsätze ergänzt
  - in `KGV.Maui/Pages/ArbeitseinsaetzeEditorPage.cs`
...
- Fachliche Wirkung dieses Abschlusses:
  ...
  - es wurde bewusst kein neuer Serien-/Schichten-DB-Unterbau neben den bestehenden Produktivpfaden aufgebaut
- Validierung im Lauf wirklich ausgeführt:
  - `dotnet build KGV.Core/KGV.Core.csproj -c Debug`
  - `dotnet build KGV.Wpf/KGV.Wpf.csproj -c Debug -clp:ErrorsOnly`
  - `dotnet build KGV.Maui/KGV.Maui.csproj -c Debug -clp:ErrorsOnly`
...
- Minimal umgesetzt:
  ...
  - in `KGV.Wpf/Views/ArbeitseinsaetzeVerwaltungEditorView.xaml`
    - neuen Button am Ende des Eingabeformulars ergänzt
  - in `ToDo.md`
    - Punkt `23` auf `teilweise` gesetzt
- Fachliche Wirkung dieses ersten Teilblocks:
  ...
  - weiter offen bleibt eine echte Serien-/Mehrschicht-Modellierung über mehrere eigenständige Zeitfenster innerhalb eines fachlichen Gesamttermins
- Validierung im Lauf wirklich ausgeführt:
  - `dotnet build KGV.Core/KGV.Core.csproj -c Debug`
  - `dotnet build KGV.Wpf/KGV.Wpf.csproj -c Debug -clp:ErrorsOnly`
  - `dotnet build KGV.Maui/KGV.Maui.csproj -c Debug -clp:ErrorsOnly`
...
- Zu Beginn wurde der echte Stand gegen `ToDo.md` und die direkt betroffenen MAUI-Dateien geprüft:
  ...
  - `KGV.Maui/UserShell.cs`
  - ergänzend der bestehende WPF-Mitgliedskontext zur fachlichen Einordnung
- Ehrlicher Istzustand vor dem Minimalfix:
  ...
  - WPF hatte diesen Einstieg im Mitgliedsbaum bereits als eigenen Kontextpunkt, MAUI im Detailscreen aber noch nicht
- Minimal umgesetzt:
  - in `KGV.Maui/Pages/MeineDatenPage.xaml.cs`
    - Button `Wartungsverträge öffnen` im Abschnitt `Wartungsverträge / Pflichtstunden` ergänzt
    - Sichtbarkeit und Aktivierung an den aktuellen Mitgliedskontext gebunden
...
- Ehrlicher Istzustand vor dem Minimalfix:
  - `MemberParzellenDetailPage` zeigte bei fehlendem Mitgliedskontext bereits eine Fehlmeldung
  - der eigentliche Detailcontainer war aber noch an `HasSelectedDetail` gebunden
  - dadurch konnten außerhalb des echten Mitgliedskontexts alte oder stale Parzellendetails sichtbar stehen bleiben
  - die Restlücke lag damit nicht im Navigationsstart, sondern in der Sichtbarkeitsbindung der Detailseite selbst
- Minimal umgesetzt:
  - in `KGV.Maui/Pages/MemberParzellenDetailPage.cs`
    - Detailcontainer von `HasSelectedDetail` auf `ShowMemberContextDetail` umgebunden
  - keine Änderung an `ParzellenContextState`, keine neue Reset- oder Schattenlogik
- Fachliche Wirkung dieses Fixes:
  - die Seite zeigt Parzellendetails jetzt nur noch im echten Mitgliedskontext
  - bei fehlendem Kontext bleibt ausschließlich die bestehende Fehlmeldung sichtbar
  - damit ist eine kleine, aber reale Navigationsinkonsistenz aus `F2` minimal behoben
- Validierung im Lauf wirklich ausgeführt:
  - `dotnet build KGV.Maui/KGV.Maui.csproj -c Debug -clp:ErrorsOnly`
  - `dotnet build KGV.Wpf/KGV.Wpf.csproj -c Debug -clp:ErrorsOnly`
...
- Ausgangspunkt dieses Laufs war ausdrücklich die Fortsetzung von `ToDo.md` für:
  - `B1. WPF-Bindingfehler MemberDTO.Name`
- Zu Beginn wurde der echte Stand in den direkt relevanten Dateien geprüft:
  - `ToDo.md`
  - `KGV.Core/Models/MemberDTO.cs`
  - `KGV.Wpf/Views/MemberSearchView.xaml`
  - `KGV.Wpf/ViewModels/MemberSearchViewModel.cs`
  - zusätzlicher Suchlauf über WPF-XAML/C# auf echte `Name`-Bindings
- Ehrlicher Istzustand vor dem Minimalfix:
  - `MemberDTO` hat im aktuellen Workspace keine Property `Name`, sondern u. a. `Nachname`, `Vorname`, `DisplayName`
  - die echte Warnstelle saß in `KGV.Wpf/Views/MemberSearchView.xaml`
  - dort verwendete die Spalte `Name` noch ein Fallback-Binding auf `Name` und `Nachname`
  - andere gefundene `Name`-Bindings, z. B. in Dokumentlisten, betrafen nicht `MemberDTO` und waren für `B1` nicht ursächlich
- Minimal umgesetzt:
  - in `KGV.Wpf/Views/MemberSearchView.xaml` die Spalte `Name` direkt auf `Nachname` gezogen
  - den nur dafür verwendeten `FirstNonEmptyConverter` aus dieser View entfernt
  - keine Änderung an `MemberDTO`, kein ViewModel-Umbau und keine künstliche Schattenproperty `Name`
- Fachliche Wirkung dieses Fixes:
